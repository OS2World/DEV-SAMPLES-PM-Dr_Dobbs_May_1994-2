...
void create_process(PROCESS *process_id,void far *entry_point())
{
  ...
  *process_id=pcs_tab+nbr_process;
  (pcs_tab[nbr_process]).context[9]=(unsigned)entry_point;
  /* if you want to use interrupts link with setjmp1.obj and add this line */
  /* (pcs_tab[nbr_process]).context[11]=0; */ /* disable it for all process */
  /* you must reset GIE flag of ST register to use interrupts */

  (pcs_tab[nbr_process]).context[4]=(unsigned)stack+PROCESS_STACK_SIZE*nbr_process; /* process stack */
  (pcs_tab[nbr_process]).context[10]=(pcs_tab[nbr_process]).context[4];
  nbr_process+=1;
  }
}
...
/*      if next_processess=NULL the higher priority ready process is    */
/*      searched, else the process is the ready process.                */
/************************************************************************/

void scheduler(next_process)
...
/* saves the context of current process */
  if (setjmp(current_process->context)==0){
    if (next_process)
    current_process=next_process;     /* the next process is next_process */
	...
	i>>=1;
      ...
      current_process=pcs_tab+n; /* next_process is the elected process */
    }
  longjmp(current_process->context,1); /* switch to the elected process */
  }
}
...
SIGNALS signals_mask;
{
  ...
  if (process->received_signals&process->expected_signals){
    ...
    /* process's priority level is higher than current_process priority */
      scheduler(process); /* switch to process */
    }
  }
}

/************************************************************************/
/* wait: place la process courante en attente des evts m       */
/*                  retourne les evenements arrives                     */
/************************************************************************/
...
SIGNALS signals_mask;
{
  ...
  /* if signals in signals_mask are not arrived */
    current_process->expected_signals=signals_mask; /* update expected_signals */
...
{SIGNALS old_signals;
  old_signals=current_process->received_signals;  /* saves arrived signals */
					/* reset signals_mask */
  current_process->received_signals=current_process->received_signals&~signals_mask;
...
{int i;                         /* loop variable */
 ...
 PROCESS pcs_ptr;               /* pointer on process contect structure */
  ...
  for(i=0;i<=nbr_process;i++){  /* for each process initialize pcs */
    ...
    ready_process|=current_process_mask;  /* the process is now ready to take the CPU */
    current_process_mask=current_process_mask>>1;  /* current_process_mask for the next process */
  }
